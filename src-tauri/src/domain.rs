...
/// 快捷请求（列表用）：与环境无关的临时接口，`url` 为完整地址（含 host）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct QuickRequestSummary {
    pub id: i64,
    /// 显示名，允许重复（无唯一键）
    pub name: String,
    pub method: String,
    pub url: String,
    /// 所属快捷请求分组（空 = 未分组，位于快捷请求根层）
    pub group_id: Option<i64>,
}

/// 快捷请求分组（列表用）：`parent_id` 为空即顶层分组
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct QuickGroupSummary {
    pub id: i64,
    /// 显示名，允许重复（无唯一键）
    pub name: String,
...
/// 快捷请求整棵树：分组（含嵌套）与快捷请求均平铺下发，由前端组树
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct QuickTree {
    /// 按创建顺序（同层即自上而下）
    pub groups: Vec<QuickGroupSummary>,
    /// 按创建时间倒序（最新的在前）
    pub requests: Vec<QuickRequestSummary>,
}
...
/// 快捷请求名称校验：不参与唯一键（允许重名），因此只要求非空、长度与可打印字符
pub fn validate_label(raw: &str) -> Result<String, String> {
    ...
    if name.chars().any(|c| c <= '\u{1f}' || c == '\u{7f}') {
