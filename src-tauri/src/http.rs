...
/// URL 的 origin（scheme + authority），用于把用户手写的完整地址记为该请求的快照 host。
/// 无 scheme（相对路径等）时返回空串。
pub fn origin_of(url: &str) -> String {
    ...
    let end = rest
        .find(|c: char| c == '/' || c == '?' || c == '#')
        .unwrap_or(rest.len());
    url[..after + end].trim_end_matches('/').to_string()
}
...
mod tests {
    ...
    fn origin_of_keeps_scheme_and_authority_only() {
        assert_eq!(origin_of("http://127.0.0.1:8080/api/login?a=1"), "http://127.0.0.1:8080");
        assert_eq!(origin_of("https://api.example.com"), "https://api.example.com");
        assert_eq!(origin_of("https://api.example.com/"), "https://api.example.com");
        // 无路径但有查询：origin 仍只保留 authority（与历史重发的 host 前缀校验对齐）
        assert_eq!(origin_of("http://h.local?x=1"), "http://h.local");
        assert_eq!(origin_of("/api/login"), "");
        assert_eq!(origin_of("{{host}}/api"), "");
    }
