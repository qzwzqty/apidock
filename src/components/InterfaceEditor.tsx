...
export function InterfaceEditor({
  ...
  /**
   * 地址编辑方式：
   * - env：host 取自环境（只读前缀），用户只填路径（项目接口）
   * - absolute：host 不继承环境变量，用户在地址栏填写完整地址（快捷请求）
   */
  urlMode?: "env" | "absolute";
  /**
   * 只保留调试：不显示「文档 / 编辑 / 调试」切换，始终处于调试模式
   * （快捷请求用；内容由发送后自动落库，故无需编辑态与保存按钮）
   */
  debugOnly?: boolean;
}) {
  const fullUrl = urlMode === "absolute";
  const [base, setBase] = useState<InterfaceFile>({ ...doc });
  const [mode, setMode] = useState<EditorMode>(debugOnly ? "debug" : (defaultMode ?? "doc"));
  ...
  const handleSend = () => {
    const sendDoc: InterfaceFile = {
      ...base,
      // 完整地址模式：host 由用户输入的地址自带，不做 {{host}} 模板化
      url: fullUrl ? base.url.trim() : normalizeUrlForSend(base.url, host),
    };
  ...
  return (
    <div className="flex h-full flex-col">
      {/* 模式标签：文档 / 编辑 / 调试（Apifox 风格）；debugOnly 时不展示 */}
      {!debugOnly && (
      ...
      {mode === "doc" ? (
        <div className="flex items-center gap-2 px-4 py-3">
          ...
          <span className="min-w-0 flex-1 truncate font-mono text-sm text-foreground" title={base.url}>
            {base.url
              ? fullUrl
                ? base.url
                : base.url.replace(HOST_TEMPLATE, host.trim() || HOST_TEMPLATE)
              : "（未设置请求地址）"}
          </span>
        ...
        <div className="flex items-center gap-2 px-4 py-3">
          ...
          {mode === "debug" && (
            <Button
              onClick={handleSend}
              disabled={fullUrl ? !base.url.trim() : !host}
              className="bg-green-600 hover:bg-green-500"
              title={
                fullUrl
                  ? "按完整地址发送（host 不来自环境，需含 http:// 或 https://）"
                  : host
                    ? undefined
                    : "当前环境未配置 host，请在环境管理中设置后发送"
              }
            >
...
export function UrlInput({
  ...
  placeholder?: string;
  /** 完整地址模式：直接编辑完整地址（含 host），不做 {{host}} 模板化 */
  full?: boolean;
}) {
  if (full) {
    return (
      <div className="flex h-9 min-w-0 flex-1 items-center overflow-hidden rounded-md border border-border bg-muted focus-within:border-ring">
        <input
          ...
          value={url}
          placeholder="http://127.0.0.1:8080/api/login"
          title="完整地址（含 host）：不继承环境变量"
          spellCheck={false}
